...
void Filtros::FiltroGaussianoUnCanal(){
    ...
    for (int i=1; i<imagen.rows-1; i++){
	    for (int j=1; j<imagen.cols-1; j++){
	    	...
	    	for (int x = -1; x < 2; x++){
	    		for (int y = -1; y<2; y++){
	    			if ((x == 0) && (y == 0)){ //Este if evita el pixel central, haciendo que el programa solo se centre en los del vecindario
	    			}else{
...
void Filtros::Erosion(){
    ...
    //se estudia el vecindario y se cumple con el elemento estructurante se cambia el pixel
    for (int i=1; i<imagen.rows-1; i++){
    	for (int j=1; j<imagen.cols-1; j++){
...
void Filtros::Dilatacion(){
    ...
    for (int i=1; i<imagen.rows-1; i++){
    	for (int j=1; j<imagen.cols-1; j++){
            ...
            promedio = (imagen.at<Vec3b>(i, j)[0] + imagen.at<Vec3b>(i, j)[1] + imagen.at<Vec3b>(i, j)[2])/3;
            //elemento estructurante
            int tamanio = 3;
            ...
            for (int fv = (i-1);fv<(i-1)+tamanio;fv++){
                int coc=0;
                for(int cv=(j-1);cv<(j-1)+tamanio;cv++){
                    vecindario[cof][coc] = (imagen.at<Vec3b>(fv, cv)[0] + imagen.at<Vec3b>(fv, cv)[1] + imagen.at<Vec3b>(fv, cv)[2])/3;
                    cout << (int)vecindario[cof][coc] << "  ";
                    coc++;
